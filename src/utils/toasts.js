"use strict";export default class Toasts{constructor(options){let defaults={position:'top-right',stack:[],offsetX:20,offsetY:20,gap:20,numToasts:0,duration:'.5s',timing:'ease',dimOld:!0};this.options=Object.assign(defaults,options)}
push(obj){this.numToasts++;let toast=document.createElement(obj.link?'a':'div');if(obj.link){toast.href=obj.link;toast.target=obj.linkTarget?obj.linkTarget:'_self'}
toast.className='toast-notification'+(obj.style?' toast-notification-'+obj.style:'')+' toast-notification-'+this.position;toast.innerHTML=`
        ...
        `;document.body.appendChild(toast);toast.getBoundingClientRect();if(this.position=='top-right'){toast.style.top=0;toast.style.right=this.offsetX+'px'}
...
if(toast.element.dataset.transitionState=='close'){return!0}
if(toast.element.querySelector('.toast-notification-close')){toast.element.querySelector('.toast-notification-close').onclick=null}
...
get dimOld(){return this.options.dimOld}}