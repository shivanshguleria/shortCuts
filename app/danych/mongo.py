...
def update_count(unique_id, country = None):
   ...
   # process data if not  Nonetype, usefull when analytics  database destroyed 
    ...
    try: 
        ...
        post.find_one_and_update({"_id": unique_id}, {"$inc": {"count": 1}, "$set": {f"analytics.{country}.count": 1, f"analytics.{country}.date.{datetime.datetime.now().date()}": 1, "v": 2}}, upsert=True)
